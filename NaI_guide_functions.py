...
background = 'NaI_Background_0deg_gain1.Spe'
...
def calibration_curve(linear, energies, channels):
    ...
    ax.legend()
    plt.title("NaI Energy vs. Channel Calibration Curve")
    plt.grid(True)
    plt.show()
...
def resolution_curve(energies, sigmas):
    ...
    ax.legend()
    plt.title("NaI Resolution vs. Energy (Polynomial Fitting)")
    plt.grid(True)
    plt.show()
...
def absolute_efficiency_curve(energies, areas, current_source_activity, source_emission_fraction):
    ...
    ax.legend()
    plt.title("NaI Efficiency vs. Energy (log-log)")
